        ...
        chrome.tabs.sendMessage(tab.id, { action: "extractText" }, function (response) {
          ...
          chrome.runtime.sendMessage({ action: "analyze", text: articleText }, function (res) {
            ...
            chrome.runtime.sendMessage({
              ...
              const hasAnyVector = vectorResults && (vectorResults.left || vectorResults.center || vectorResults.right);
              const hasAllVector = vectorResults && vectorResults.left && vectorResults.center && vectorResults.right;

              if (hasAllVector) {
                renderResults(biasData, vectorResults, tab);
              } else {
                var query = buildRelatedNewsQuery(biasData, tab);
                ...
                chrome.runtime.sendMessage(
                  ...
                  function (newsRes) {
                    ...
                    renderResults(biasData, mergePerspectiveData(vectorResults, newsData), tab);
                  }
                );
              }
            });
...
function buildRelatedNewsQuery(biasData, tab) {
  const summary = biasData && biasData.article_summary ? String(biasData.article_summary).trim() : "";
  if (summary) return summary.slice(0, 220);

  const searchQuery = biasData && biasData.search_query ? String(biasData.search_query).trim() : "";
  if (searchQuery) return searchQuery;

  return tab ? (tab.title || "news").replace(/\s*[-|]\s*[^-|]+$/, "").trim() : "news";
}
...
function renderResults(biasData, newsData, tab) {
  ...
  const parsedScore = (biasData && biasData.bias_score !== undefined && biasData.bias_score !== null)
  ...
  const scoreLabel = initialScore !== null
    ? ((initialScore >= 0 ? "+" : "") + initialScore.toFixed(1))
    : "…";
